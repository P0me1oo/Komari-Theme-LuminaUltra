...
LuminaUltra **v1.6.4** 将独立资产悬浮入口移到首页顶部快捷栏，与主题设置放在一起；修复手机端访客信息卡片偏左和详情单列的问题，并统一香港、台湾、澳门的位置名称。基于已同步的 [LuminaPlus v1.3.2](https://github.com/shanyang242/Komari-Theme-LuminaPlus/releases/tag/v1.3.2)，完整变更见 [v1.6.4 更新说明](.github/release-notes/v1.6.4.md)。
...
- 原“显示资产悬浮按钮”设置沿用保存字段 `showCostSummaryFloatingButton`，升级后自动用于顶部快捷入口，无需重新设置。手机端不再显示独立的底部资产按钮；顶部快捷栏空间不足时按组换行，首页内容随之下移。
