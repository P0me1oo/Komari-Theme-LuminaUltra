...
LuminaUltra **v1.9.0** 把访客费用开关升级为“费用可见范围”三档单选，新增对所有人（含管理员）隐藏价格的选项，隐藏时管理员仍可从资产快捷入口核对明细。其余功能保持不变，完整变更见 [v1.9.0 更新说明](.github/release-notes/v1.9.0.md)。
...
登录后，配合 [Komari-IP-Info](https://github.com/shanyang242/Komari-IP-Info) 插件可查看“IP 信息”页签，支持 IPv4/IPv6 信息与全球延迟检测；插件不可用、地址被排除或中国区域时不显示入口。

大卡片和小卡片的流量额度旁新增每月重置提示，重置日根据到期日期推导。iOS 主屏独立模式支持在数据页面顶部下拉刷新。
...
在主题管理的“服务器花费”中，通过“费用可见范围”三档单选统一控制费用展示：

- **所有人可见**：访客和登录用户都能看到价格与资产金额，新安装的默认值。
- **仅登录可见**：访客看不到所有卡片和列表中的价格、首页资产金额和评级、资产页入口及费用提醒入口；直接打开 `/assets` 会返回首页，且不请求费用汇率。价格排序选项隐藏，已有价格排序偏好回退到默认权重顺序。登录用户不受影响。
- **所有人隐藏**：上一档的隐藏效果对登录管理员同样生效，首页不再出现任何价格、资产概览卡和费用提醒入口。管理员仍可从首页右上角的资产快捷入口打开 `/assets` 核对价格、到期与续费明细；访客访问会返回首页。
- 三档都不影响节点到期时间和剩余天数，它们由小卡片的“显示到期时间”单独控制。
- 升级兼容：尚未保存新设置时，旧版“向未登录访客公开费用”为关、或更旧的“向访客开放资产统计”“显示服务器价格”任一为关，都迁移为“仅登录可见”，否则为“所有人可见”；旧配置不会自动迁出“所有人隐藏”，需要手动选择。保存后以新值为准，旧字段从配置中清除。
