...
LuminaUltra **v1.8.0** 同步 [LuminaPlus v1.3.3](https://github.com/shanyang242/Komari-Theme-LuminaPlus/releases/tag/v1.3.3)，统一访客费用公开开关，新增实例 IP 信息、流量重置提示与 iOS PWA 下拉刷新。保留 V4/V6 标签开关、三网探测点批量配置等现有功能，完整变更见 [v1.8.0 更新说明](.github/release-notes/v1.8.0.md)。
...
在主题管理的“服务器花费”中，通过“向未登录访客公开费用”统一控制费用展示：

- 新安装默认开启。升级时，如果尚未保存新开关，旧版“向访客开放资产统计”或“显示服务器价格”任一关闭，新开关也会关闭；保存新开关后，以新值为准。
- 关闭后，访客看不到所有卡片和列表中的价格、首页资产金额和评级、资产页入口及费用提醒入口；直接打开 `/assets` 会返回首页，且不请求费用汇率。价格排序选项隐藏，已有价格排序偏好回退到默认权重顺序。
- 登录用户仍可查看费用；节点到期时间继续显示。旧版两个开关已从设置页移除，卡片价格不再单独对登录用户隐藏。
