...
LuminaUltra **v1.6.0** 新增资产统计访客开关、离线优先与高负载排序，以及参考 Emerald 的访客信息卡片。基于已同步的 [LuminaPlus v1.3.2](https://github.com/shanyang242/Komari-Theme-LuminaPlus/releases/tag/v1.3.2)，保留现有总览卡片、价格与到期时间设置、Ping 丢包率图表和背景动效。完整变更见 [v1.6.0 更新说明](.github/release-notes/v1.6.0.md)。
...
| --- | --- | --- |
| 离线优先 | 离线节点在前，在线节点在后；各组内按后端权重排列 | 切换为在线优先 |
...
参考 [komari-theme-emerald](https://github.com/Tokinx/komari-theme-emerald) 的访客卡片，在首页、节点详情、资产页和流量页底部显示当前访问者的信息，沿用 LuminaUltra 的浅色、深色和浮层样式。

- 默认开启，可在主题管理中关闭“显示访客信息卡片”。主题管理页本身不显示卡片。
- 桌面端折叠时显示来源、部分遮盖的 IP 和浏览器；手机端保留来源，点击后展开。
- 展开后显示来源、完整 IP、设备、浏览器、运营商和本次页面访问时间。可再次点击、点击卡片外部或按 Esc 收起，支持键盘操作。
